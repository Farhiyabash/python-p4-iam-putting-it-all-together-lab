...
class Signup(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            #after the user is signed we store their information in a session

            # Store the user_id in session
            session['user_id'] = new_user.id  # Make sure to save the user ID
...
class CheckSession(Resource):
    def get(self):
        #check if the key user_id is in the session object
        #A session is a special object that allows one to stroe information about the user across different request. is is like temporary storage space that can hold uer data while navigating the application
        # if the user is logged in the user ID will be stores here
...
class RecipeIndex(Resource):
    def get(self):
        ...
        if 'user_id' in session and session['user_id'] is not None:
            # Query for recipes associated with the logged-in user
